<template>
      <div class="w-full container ">
            <h1 class="font-bold text-center md:text-[39px] text-[30px] my-16">Product Filter</h1>
    
        <!-- Navbar-like category list -->
        <ul class="flex justify-between items-center my-8 border rounded-lg p-4 flex-wrap">
          <li
            v-for="category in categories"
            :key="category"
            :class="[
          'cursor-pointer px-4 py-2  rounded-lg my-8 md:my-0',
        ...
        <div v-if="filteredProducts.length" class="grid grid-cols-1 md:grid-cols-3 mx-auto">
          <div v-for="product in filteredProducts" :key="product.id" class="product-card mx-auto border rounded-lg p-4">
...
<span class="relative inline-flex rounded-full h-3 w-3 bg-pink-500 ">
            ...
            <!-- <img :src="product.images[0]" alt="Product Image" class="w-[320px]  h-[220px] object-cover    rounded-lg   hover:opacity-50" /> -->
